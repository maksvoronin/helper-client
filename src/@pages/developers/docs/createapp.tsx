...
const CreateApp: FC<PageProps> = observer(({ title }) => {
  return (
    <DevelopersLayout title={title}>
      ...
      <FormText>Создание приложения происходит по <Link to={"/developers/app/create"}>ссылке</Link>. В этой форме Вам необходимо указать название приложения, описание, где и как им можно воспользоваться. После успешного создания Вам будет доступен собственный токен приложения, который нужно будет указывать при выполнении запросов.</FormText>
