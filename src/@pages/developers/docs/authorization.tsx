...
const Authorization: FC<PageProps> = observer(({ title }) => {
  return (
    <DevelopersLayout title={title}>
      ...
      <Code>https://helper.voronin.xyz/login/application?app_id=&lt;APPLICATION_ID&gt;</Code>
      <FormText>
        ...
        перенаправлять пользователя, чтобы Вы могли получить токен в параметре <code>user_token</code>
      </FormText>
      <FormText>
      ...
      <Code>https://helper.voronin.xyz/login/application?app_id=06dcSOME_APP_ID</Code>
