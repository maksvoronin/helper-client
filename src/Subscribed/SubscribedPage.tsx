...
const SubscribedPage = ({ title }: DefaultPage) => {
  ...
  useEffect(() => {
    if (store.user) {
      ...
      store.user && store.user.subscribedSystems && store.user.subscribedSystems.forEach((e: any) => {
        $api.get(`${config.API}/system/get?id=${e}`).then(({ data }) => setUserSystems(prev => [...prev, data.data]));
      });

      store.user && store.user.subscribedSystems && store.user.subscribedComments.forEach((e: any) => {
          ...
          <h1>Системы</h1>
          {
            userSystems && userSystems.map((e: any) =>
              <div key={e._id}>
                <b>{e.name}</b>
                <ul>
                  {comments.map(r => r.system === e._id && r.visible && <li key={r._id}><Link to={`/comment/${r._id}`}>{r.content}</Link></li>)}
                </ul>
              </div>
            )
          }
          ...
          <h1>Замечания</h1>
          {
            userComments && userComments.map((e: any) =>
              <div key={e._id}>
                <b>{e.content}</b>
                <ul>
                  {decisions.map(r => r.comment === e._id && r.visible && <li key={r._id}><Link to={`/decision/${r._id}`}>{r.content}</Link></li>)}
                </ul>
              </div>
            )
          }
